...
const EMPTY_ENTRY_SOURCES:ContentSource[]=[];
export default function App({returnLabel="返回观星台",entrySources=EMPTY_ENTRY_SOURCES}:{returnLabel?:string;entrySources?:ContentSource[]} = {}) {
  const [entry, setEntry] = useState(() => {
    const initial = getInitialEntry();
    return { ...initial, returnUrl: initial.returnUrl ?? "/observatory" };
  });
  const [query, setQuery] = useState(entry.query);
  const [sourceEntryQuery] = useState(entry.query);
  ...
  const selectedSourceKey = useMemo(
    ...
    // Only source text changes invalidate highlights; other answer metadata must not.
  ...
  useEffect(
    () =>
      registerObservatoryEntry((next) => {
        ...
        beginTrip();
        setDiscoveryMode("hot");
        setEntry({ ...next, returnUrl: next.returnUrl ?? "/observatory" });
        setQuery(next.query);
  ...
  useEffect(() => {
    ...
    wormholeResult.current = null;
    const sourceEntry = !prepared && entrySources.length > 0 && query === sourceEntryQuery && retry === 0;
    const animateSearch = !prepared && !sourceEntry && !reducedMotionRef.current && !pendingVisit.current && !!sceneSnapshot.current?.questions.length;
    ...
    if(sourceEntry) {
      const sourceQuestions:Question[]=entrySources.map((source,index)=>({
        id:`reading-source-${index}`,kind:'topic',title:source.title,excerpt:source.summary,keywords:source.tags??[],relevance:1-index*.03,color:['#d6b77d','#9cbfb8','#b5a2d4','#d79b96'][index%4],answersExpanded:true,
        answers:[{id:source.remoteId??source.id,title:source.title,author:source.author,sourceName:source.source,curated:source.kind==='curated',excerpt:source.summary,paragraphs:[source.summary,...(source.readingGuide?[`阅读导引：${source.readingGuide}`]:[])],url:source.url,isExcerpt:true,relevance:1}]
      }))
      setData({query,keywords:[],questions:sourceQuestions,source:'curated',fetchedAt:new Date().toISOString(),notice:'来自这次旅程的真实阅读来源。当前展示导引或摘要，完整内容见原文链接；可以搜索新问题，继续探索知乎讨论。'})
      setSelectedQuestionId(sourceQuestions[0].id);setSelectedAnswerId(sourceQuestions[0].answers[0].id);setLoading(false)
      return () => { controller.abort(); clearTimeout(collapseTimer); }
    }
  ...
  }, [query, retry, discoveryMode, notify, entrySources, sourceEntryQuery]);
  ...
  }, [selectedSourceKey, selectedQuote, selectedAnswer]);
  ...
  return (
    ...
    >
      <div
        ...
        inert={modalActive || wormholeActive}
      ...
      <div className="interface" inert={modalActive || wormholeActive} aria-hidden={wormholeActive}>
        ...
        {!loading &&
          (answerLoading ||
            ...
            detailError) && (
            <div className="orbit-status" role="status">
              {answerLoading || highlightLoading || detailLoading ? (
                <>
                ...
                <>
                  <Info size={14} />
                  <span>{detailError || (level === 1 ? answerError : highlightError)}</span>
                  <button
                    onClick={() =>
                      detailError
        ...
        {level > 0 && selectedQuestion && (
          <div className="context-actions" aria-label="当前内容操作">
            ...
            {level === 2 && (
              <>
                <i />
                <button aria-label={selectedAnswer?.isExcerpt && !selectedAnswer.workId ? "打开摘要与来源" : "打开原文阅览"} onClick={() => openReader()}>
                  <BookOpen size={15} />
                  <span>{selectedAnswer?.isExcerpt && !selectedAnswer.workId ? "摘要" : "原文"}</span>
                  <kbd>F</kbd>
                </button>
              </>
            )}
      ...
      <div className="wormhole-flow" inert={modalActive}>
        {wormhole.phase === "idle" && (
        ...
        >
          ...
          {drawer === "return" && (
            <>
              ...
              <p className="modal-subtitle">
                {tripStatus || "足迹尚未能返回观星台，请稍后重试。收藏与思考仍保存在个人空间。"}
              </p>
              <div className="button-row">
        ...
        <Modal title="是否导出漫游足迹" className="trip-exit-modal" onClose={() => setTripExit(null)}>
          ...
          <div className="trip-exit-actions">
            <button className="primary-button" onClick={() => finishTrip(true)}>
              <Download size={16} />{tripExit === "return" ? `导出并${returnLabel}` : "导出并结束旅行"}
            </button>
            <button className="secondary-button" onClick={() => finishTrip(false)}>
              {tripExit === "return" ? `不导出，${returnLabel}` : "不导出，结束旅行"}
            </button>
