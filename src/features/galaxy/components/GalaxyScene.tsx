...
export default function GalaxyScene(props: GalaxySceneProps) {
  ...
  const { voyage: currentVoyage, onVoyageReady } = props;
  useEffect(() => {
    if (!webglAvailable && currentVoyage?.phase === "birth" && !currentVoyage.ready)
      onVoyageReady?.(currentVoyage.id);
  }, [webglAvailable, currentVoyage, onVoyageReady]);
  ...
  const layoutSignature = props.questions
    ...
    .join("|");
  // Rebuild owned GPU resources only when source identities change, not while detail text fills in.
  ...
  useEffect(() => {
    ...
    // Current text is read live below; this effect only owns identity transitions.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const sourceSubtitle = (question: Question) =>
    question.kind === "topic"
      ? `主题聚合 · ${question.answers.length} 份材料`
  ...
  return (
    ...
    >
      ...
      {layers.map((layer) => {
        ...
        const summaryOnly = !!answer?.isExcerpt && !answer.workId;
        const readingSource = summaryOnly ? "摘要" : "原文";
        const questions =
        ...
        const text = (value: string) => (
          <StellarText
            text={value}
            content={<RichText text={value} inline />}
            ...
            {layer.stage > 0 && question && (
              <>
                <button
                  ...
                  aria-label={
                    layer.stage === 2 ? `打开中心文章${readingSource}` : "聚焦星系核心问题"
                  }
                  onClick={() => {
                ...
                >
                  ...
                  {layer.stage === 2 && answer ? (
                    <button
                      ...
                      aria-label={`阅读${readingSource}：${answer.title}`}
                    >
                      {text(answer.title)}
                    </button>
                  ) : (
                  ...
                  {layer.stage === 2 && (
                    <button
                      ...
                      onClick={() => props.onOpenReader()}
                    >
                      {summaryOnly ? "展开摘要与来源" : "展开原文阅览"} <kbd>F</kbd>
                    </button>
                  )}
                    ...
                    <div className="galaxy-label-body">
                        ...
                        {isQuestion
                          ? q?.kind === "topic"
                            ...
                            : q?.kind === "article"
                              ...
                              : "QUESTION"
                          : isParagraph
                            ? `${summaryOnly ? "SUMMARY" : "EXCERPT"} ${String(page * pageSizeRef.current + index + 1).padStart(2, "0")}`
                            : question?.kind === "topic" ||
                                question?.kind === "article"
                              ? "ARTICLE"
                              : "ANSWER"}
                      ...
                      <span className="galaxy-label-meta">
                        {isQuestion
                          ? sourceSubtitle(q!)
                          : highlight
                            ? `${readingSource}第 ${highlight.paragraphIndex + 1} 段 · ${planetKindNames[getPlanetKind(a!.id, page * pageSizeRef.current + index)]}${selected ? " · 已选中" : ""}`
                            : a?.author}
                      </span>
                      {!highlight ? (
                        <button
                        ...
                        <button
                          className="galaxy-label-enter"
                          onClick={enter}
                          aria-label={`在${readingSource}中阅读第 ${highlight.paragraphIndex + 1} 段`}
                        >
                          在{readingSource}中阅读<span aria-hidden="true">↗</span>
                        </button>
                      )}
                    </div>
                  </div>
