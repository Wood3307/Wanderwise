import { sharedCollection, saveSharedCollection, sharedReflections, saveSharedReflections } from '../../personal/galaxyBridge';
/* eslint-disable no-control-regex -- Sanitize control bytes in downloaded filenames. */
...
function write<T extends { id: string }>(key: string, items: T[], limit: number, validate: RecordValidator<T>, identity: (item: T) => string = item => item.id, share?: (items:T[])=>void): boolean {
  try {
    ...
    if(usePersonalStore.getState().ready && share){
      share(normalized);
      // The IndexedDB-backed store is authoritative; an old localStorage quota does not block it.
      try{window.localStorage.setItem(key,serialized)}catch{/* The original backup stays intact. */}
      return true;
    }
...
export const loadCollection = (): SavedItem[] => sharedCollection() ?? read(KEYS.collection, MAX_RECORDS.collection, savedItem, savedIdentity);
...
function markdown(value: string): string {
  return value.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')
    // eslint-disable-next-line no-useless-escape -- Preserve the upstream export escaping character set.
    .replace(/([\\`*_{}\[\]()#+.!|~-])/g, '\\$1');
}
