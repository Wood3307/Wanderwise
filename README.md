...
中文采用本地提供的朱雀仿宋。出现 Markdown 或 LaTeX 的段落按语法排版，普通文字保持原样；支持加粗、斜体、列表、引用、代码与表格，以及 `$…$`、`$$…$$`、`\(…\)`、`\[…\]` 数学公式。星图使用紧凑排版，原文窗口显示完整块排版；选段、收藏和思考仍引用原始文本。文字星尘从实际字形、公式上下标、分数线与根号采样，不把语法标记画成碎片。

再次搜索时，旧场景会保留到吸入结束：星系加速沿弧线向背景黑洞收拢，产生潮汐拉伸与解离星尘；等待期间只保留吸积光晕。新结果通过一次超新星闪燃、扩张光环与星尘喷发涌现。请求与动画并行，连续搜索仅显示最新结果；失败和空结果仍可重试。开启「减少动态效果」时直接切换结果。
