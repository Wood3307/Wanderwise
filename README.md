...
星系包含棒旋、螺旋、椭圆、透镜和不规则五种几何形态；行星包含海洋、岩石、气态、环状、冰质和熔岩等程序化表面，带恒星方向光照、昼夜明暗、星环和缓慢公转。形态设计参考 [NASA 星系分类](https://science.nasa.gov/universe/galaxies/types/) 与 [NASA 行星资料](https://science.nasa.gov/solar-system/planets/)，比例和轨道为阅读体验调整，不是天体物理数值模拟。移动端每页显示两条文字，桌面每页四条；其余天体仍可点击定位。减少动态效果会停止自动公转与自转，手动拖动仍然有效。
...
- `src/components/space/`：星系形态、真实行星模型、世界坐标投影及文字避让。
