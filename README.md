...
星系包含棒旋、螺旋、椭圆、透镜和不规则五种几何形态，宏观位置按 X/Y/Z 三个轴和常用斜视方向优化。位置固定在世界空间中，不会随视角重新排列；相同问题集合改变排序时，星系也会保留对应的位置。

恒星使用湛蓝、银白、紫辉、琥珀四种艺术风格，包含不同光球纹理、日冕、磁环与可选尘埃盘。同一篇回答在第二、三层使用一致的恒星外观。行星使用异质流体、岩质、气态、环状、冰质和熔岩等程序化表面，每类有不同配色；流体世界没有地球式海陆或极冠。

深层空间中有少量低亮度彗星，沿外围轨迹偶尔出现。星系会缓慢自转，回答恒星与旋臂使用同一变换；悬停或聚焦文字、输入、打开阅览窗口、拖动时，自动运动暂停，恢复后不跳跃。减少动态效果会关闭彗星并冻结自转、公转和恒星活动，手动拖动仍然有效。
